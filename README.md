...
```
cd stream && python live-stream.py --out desktop
```
Or from repo root: `AZAN_TV_WORKDIR=/path/to/workdir PYTHONPATH=stream python stream/live-stream.py --out desktop --conf /path/to/workdir/config.json`
...
The app uses a dedicated **working folder** for user data (config, media, logs). When running from source, secrets and certs in **keys/** are copied into the working folder so the stream can use them.
...
- runtime files/logs (`logs/`, `tmp/`, `ffplayout.log/`)
- keys/certs copied from **keys/** when present (`client_secret.json`, `user-oauth2.json`, `server.crt`, `server.key`)

Default working folder:
- **Linux:** `~/.local/share/azan-tv` (or `$XDG_DATA_HOME/azan-tv` when set)
- **macOS:** `~/Library/Application Support/azan-tv`
